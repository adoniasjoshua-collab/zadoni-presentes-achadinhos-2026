...
try {
  ...
  for(const width of [360,390,768,1440]) {
    ...
    await evaluate("document.querySelector('.depoimentos-artes').scrollIntoView()");
    const state=await evaluate(`(() => {const g=document.querySelector('.depoimentos-artes-galeria'); const before=g.scrollLeft; g.scrollLeft=400; return {overflow:document.documentElement.scrollWidth>innerWidth+1,brokenImages:Array.from(document.images).filter(i=>!i.naturalWidth).map(i=>i.src),faqOpen:g.scrollLeft>before,h1:document.querySelectorAll('h1').length,cards:g.querySelectorAll('figure').length}})()`);
    await evaluate("document.querySelector('.depoimentos-artes-galeria').scrollLeft=0");
